...
$calcAmountPaidDB = $amount_paidDB + $investmentAmount;
    ...
    if($dashRCount < 1){
    ...
    }elseif ($depoAmount >= $investmentAmount){
        echo 12; //There is sufficient fund in your depsoit account for this trade, kindly invest using deposit account.
        ...
        $profitBalanceDebit = $amount_paidDB - @$investmentAmount;
        ...
        if($totalAsset < $investmentAmount){
            ...
            if ($paidBefore > 0) {
                $divideMinInvest = $investmentAmount / 2;
//                $divideCapital = $amount_paidDB / 2;
            if($depoAmount >= $divideMinInvest){
                if ($investmentAmount >= $amount_paidDB) {

                    $subtractInvstAmountFromDeposit = $investmentAmount - $depoAmount;
                    $debitProfitAcc = $subtractInvstAmountFromDeposit -  $amount_paidDB ;//result will be remaining deposit balance

                    $subDepo = $investmentAmount - $depoAmount;
                    $strippedResult = str_replace("-"," ", $subDepo);
                    $increaseAmountInvested = $amount_paidDB + $depoAmount;//add amount paid in database  to investment amount
//                    echo $subDepo;

//                    $subtractInvstAmountFromProfit = $investmentAmount - $amount_paidDB;
//                    $debitDepositAcc = $investmentAmount - $depoAmount;//result will be remaining deposit balance
...
//                    $increaseAmountInvested = $amount_paidDB + $investmentAmount;//add amount paid in database  to investment amount
                    ...
                    $paid_query1 = "UPDATE paid SET plan = '$cPackage', approved = '1', fund_method = '$fromAccount', amount_paid = '$increaseAmountInvested', amount_paid_increment = '0', amt_percentage = '$amt_paid_percentage1', date_of_payment = NOW(), last_withdrawal_date = NOW(), date_increment = NOW() WHERE user_id='$users_id'";
...
//                         include('mailingSuccessfulInvestment.php');
                    ...
                    $subtractInvstAmountFromDeposit = $investmentAmount - $depoAmount;
                    $debitProfitAcc = $amount_paidDB - $subtractInvstAmountFromDeposit;//result will be remaining deposit balance

                    $increaseAmountInvested = $amount_paidDB + $subtractInvstAmountFromDeposit;//add amount paid in database  to investment amount
