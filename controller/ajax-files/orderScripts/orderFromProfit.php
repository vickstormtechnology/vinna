...
if($investmentAmount <= $dailyIncome){
    if($dashRCount < 1){
    ...
    }elseif ($lockInvestingViaProfit == 1){
        ...
        if ($paidBefore > 0) {
            ...
            $paid_query1 = "UPDATE paid SET plan = '$cPackage', approved = '1', fund_method = '$fromAccount', amount_paid = '$increaseAmountInvested', amount_paid_increment = '$profitBalanceDebit', amt_percentage = '$amt_paid_percentage1', date_of_payment = NOW(), last_withdrawal_date = NOW(), date_increment = NOW() WHERE user_id='$users_id'";
